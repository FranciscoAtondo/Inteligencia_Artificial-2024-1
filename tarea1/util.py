...
def evaluatePredictor(examples: Tuple[FeatureVector, int], predictor: Callable) -> float:
    """
...
def outputErrorAnalysis(examples: Tuple[FeatureVector, int], featureExtractor: Callable, weights: WeightVector, path: str):
...
def generateClusteringExamples(numExamples: int, numWordsPerTopic: int, numFillerWords: int) -> List:
    """
...
def outputClusters(path: str, examples: List[Dict[str, float]], centers: List, assignments: List):
    """
